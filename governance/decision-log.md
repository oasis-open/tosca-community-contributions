...
| P6 | **Incorporate the agreed changes into the upstream community profiles rather than holding them in the proposal document.** The abstract-profile proposals have accumulated in a document for two months; once agreed they go into the profile files, where the whole community sees them and can object against working YAML rather than prose. Chris to demonstrate the substituting templates that exercise them if anything is unclear. **Agreed 2026-09-02** (Roberto concurring). This is what moves N8 and I8 forward. | 2026-09-02 |
...
| N9 | **One containment relationship kind, one requirement name.** `host` is the requirement name for deployment layering at every level, declared once on `Base` and refined by each child, with the **capability** saying what kind of thing is being placed. The requirement name is the substance of the decision; whether `HostedOn`, `RunsOn` and `AvailableOn` collapse into one relationship type is deliberately **left open** — Roberto's original three types were proposed when it was not yet known whether each would need distinct properties or attributes, and months of use show they do not, so the base `HostedOn` is what the requirement declares for now. A derived relationship type is reintroduced if and when one needs properties, attributes or interface inputs of its own, without disturbing the requirement name. **Corollary:** constrain on **one side only** — either the valid capability types or the valid relationship types, not both — since constraining both over-constrains and nothing binds. This corollary confirms the resolution drafted for I16(a). **Agreed 2026-09-02.** Proposal in Section 2.3 of the abstract-profile doc; supersedes the harmonization half of N6. (Chris) | 2026-09-02 |
| N10 | **Never use a base capability or relationship type directly in a profile; always derive one that names the intent.** A derived type that adds no properties is still worth declaring: it states what the connection is for, and it keeps discrimination possible if a second type is later derived from the same parent — a filter on `Partner` cannot separate two kinds of association, a filter on `Service` can. The counter-example is TOSCA v1.3's root `dependency` requirement, which authors used in place of expressing what they actually meant. **Agreed 2026-09-02**, raised by Roberto against the property-free `Service` capability of Section 2.6 and accepted as the reason to keep it. Belongs in [`design-guide.md`](../profiles/community/tosca/docs/design-guide.md). (Chris) | 2026-09-02 |
| N11 | **One interaction port on `Application`, specialized per kind.** Application-to-application interaction is a general concept and belongs on the abstract `Application` type, not on two of its concrete children. Declare a property-free `Service` capability on `Application`, derived from `Partner`; rederive `Endpoint` from it for the network-addressable case; rederive `InteractsWith` from `AssociatesWith` rather than `DependsOn`; and drop the constraint that both ends be nodes of the same type. **Agreed 2026-09-02** — *"more correct than the previous situation, in which we had specialized interactions between derived nodes of the same type, and only on a subset of them, not all"* (Roberto). Proposal in Section 2.6, reasoning in Problem 7. (Chris) | 2026-09-02 |
| N12 | **`SingleHostApplication` becomes `ServerApplication`; `processes` is dropped or made optional.** Name the type for the platform it targets, consistent with `MicroServiceApplication` and `ServerlessApplication`, rather than for a cardinality it does not constrain; express cardinality as a `count_range` on the placement requirement instead. The `processes` property is dropped: naming the executables run on a host is below the System View, and the property collides with the `processes` **requirement** inherited from `Application`. If a need for it returns, two options were raised rather than restoring the name — carry the list in the inherited **`implementation-details`** property (Roberto's suggestion, and the preferred one, since it keeps the technology-specific detail opaque and parsed at the layer that understands it), or declare a differently-named property such as `packages`. **Agreed 2026-09-02.** Proposal in Section 2.7, reasoning in Problem 4. (Chris) | 2026-09-02 |
...
| D13 | **Credential references belong in `core` as two data types, and each node type states the vocabulary of credential kinds it accepts.** `CredentialRef` carries a mandatory reference to where the material is retrieved and an optional `name`; `NamedCredentialRef` derives from it and refines `name` to mandatory, for the credentials that identify a principal. A node that needs credentials declares a **map** of them, and each derived type refines the map's `key_schema` to the kinds it accepts — `ServerPlatform` to SSH key or SSH password, `VirtualizationPlatform` to token or cloud account. The key *is* the kind, so no entry carries a type field, and §9.4's rule that a refinement's validation applies in addition to the parent's means a derived type can only narrow the vocabulary. The material itself is never in the model. In the reference implementation, the only component that has to know a vocabulary is the artifact processor, which reads the map to find the credential an artifact needs. **Agreed 2026-09-02** — *"I agree with this approach, because it's general, and it applies to most of the cases"* (Roberto) — realizing D11 in concrete types. Proposal in Sections 2.1 and 2.4 of the abstract-profile doc; this is the work I8 holds the `0.1` for. (Chris) | 2026-09-02 |
