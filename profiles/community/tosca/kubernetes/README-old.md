...
TOSCA support for Kubernetes is guided by the following three
objectives:
...
1. Given that TOSCA's main benefit is its ability to define service
   ...
   interactions.
2. On the other hand, TOSCA must also maintain flexibility for
   Kubernetes experts. It must provide the ability to express
   low-level Kubernetes concepts and support **bottom-up** designs
   that align with Kubernetes resources, as losing this functionality
   could create obstacles for existing Kubernetes users.
3. TOSCA support for Kubernetes should include building blocks that
   implement **standard architectures** for applications and that
   simplify configuration for service designers by avoiding
   variability in configurations.

TOSCA profiles for Kubernetes must balance high-level abstractions
with low-level Kubernetes concepts since users might need both
approaches depending on their workflows and company requirements.
...
details. Furthermore, a collection of substituting templates can be
provided that implement *standard architectures* for implementing
different types of microservices. For example, microservices in the
Online Boutique example use environment variables to store information
needed to establish connectivity. While the use of environment
variables is specific to the application and not mandated by
Kubernetes, this approach could be *standardized* as a *best practice*
in the substituting templates.
...
> Tal has expressed concerns about the limitations of substitution
  mapping when modeling complex microservices with multiple
  relationships. He has previously proposed an alternative approach
  based on mapping *facades".
...
## Modeling Kubernetes Resources using TOSCA

When trying to represent Kubernetes resources using TOSCA types, the
following challenges need to be addressed:

1. Kubernetes does not explicitly define relationships. Instead,
   relationships are implicit and *emerge* as a result of matching
   labels. TOSCA allows service designers to make implicit Kubernetes
   relationships explicit which gives them better control over large
   applications. The TOSCA Profile for Kubernetes must identify
   different types of *implicit* relationships in Kubernetes and
   provide the corresponding *explicit* relationship types.
2. Kubernetes defines different controllers for managing
   workloads. While at the core, all workloads run Pods, there are
   various ways to manage these Pods. For example:
   - The recommended mechanism for managing workloads is to use
     Deployments, which manage the full lifecycle of sets of Pods,
     including updates and rollbacks.
   - Deployments manage ReplicaSets that handle scaling and error
     recovery for Pods. It is possible to deploy ReplicaSets directly
     without a *controlling" deployment
   - Although not recommended, it is also possible to instantiate
     individual Pods directly.

   Since Deployments, ReplicaSets, and Pods can each be deployed
   ("orchestrated") separately, it would be natural to create
   corresponding TOSCA node types for each of them and each of these
   node types should then define their own interface operations (and
   associated implementations).

3. However, since Deployments need information about the ReplicaSet
   they control, and ReplicaSets need information about their Pods, it
   would also be natural to model the *hierarchical* relationships
   between these entities (i.e., Deployments control ReplicaSets that
   control Pods) using TOSCA relationships. This means that a service
   template that defines Deployment nodes must also include the
   corresponding ReplicaSet and Pod nodes.

   However, if a Deployment node is *orchestrated*, Kubernetes will
   automatically instantiate the corresponding ReplicaSet and Pod
   nodes. In that case, how do we prevent the Orchestrator from
   invoking the interface operations for the ReplicaSet and Pod nodes?
4. An alternative approach could use properties (and corresponding
   TOSCA *data types*) to model the ReplicaSet in a Deployment node
   and to model the Pods in a ReplicaSet node. However, this
   introduces a new challenge: Since Service nodes reference Pods and
   not Deployments or ReplicaSets, there would be no elegant way to
   model the relationship between Service nodes and workloads in a
   general-purpose way.
5. Tal proposed one more alternative, which is to use labels as the
   encapsulating concept: Tal suggested considering labels as a
   higher-level concept in TOSCA, potentially as a node type or a
   relationship type, to capture the logical connections between
